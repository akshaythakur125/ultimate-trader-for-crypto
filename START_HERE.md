...
- Doctor packet status: GENERATED / FAILED
- Final decision: DO_NOT_TRADE / WAIT / MANUAL_REVIEW_ONLY
- Next required action

The operator **automatically generates** the doctor daily packet after
every run — no extra commands needed:

```
cat deploy_results/doctor_daily_packet.txt
```

If the packet fails, the operator still completes — the summary shows
the failure reason.
...
## Final Daily Packet (auto-generated by operator)

The operator generates this automatically. Just read:

```
cat deploy_results/doctor_daily_packet.txt
```

Emergency manual regenerate:
