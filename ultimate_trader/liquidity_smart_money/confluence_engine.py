...
class ConfluenceEngine:
    ...
    def analyze(
        ...
        if orderflow_bias:
            of_score = 15.0 if orderflow_bias in ("BUYER_AGGRESSION", "SELLER_AGGRESSION") else 0.0
        ...
        bias, total = self._determine_bias(breakdown)
        ...
        permission = self._determine_permission(score, reasons_against, bias)
    ...
    def _determine_bias(self, breakdown: dict[str, float]) -> tuple[DirectionalBias, float]:
        total = sum(breakdown.values())
        if total < 20:
            return DirectionalBias.NEUTRAL, total
        return DirectionalBias.LONG if total >= 20 else DirectionalBias.NEUTRAL, total
    ...
    def _determine_permission(self, score: float, risks: list[str], bias: DirectionalBias) -> TradePermission:
        if score < 20:
            return TradePermission.BLOCK
        if len(risks) >= 3:
            return TradePermission.CAUTION
        if bias == DirectionalBias.NEUTRAL and score < 40:
            return TradePermission.BLOCK
        if score < 40:
            return TradePermission.CAUTION
